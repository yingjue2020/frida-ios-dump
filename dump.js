...
function get_all_app_modules() {
    let app_modules = new Array();
    let all_modules = Process.enumerateModulesSync();
    for (var i = 0; i < all_modules.length; i++) {
        if (all_modules[i].path.indexOf(".app") != -1) {
            console.log(`[frida-ios-dump:js]${i} ${all_modules[i].path}`);
            app_modules.push(all_modules[i]);
        }
    }
    return app_modules;
}
...
function dump_module(target_module) {

    let documentDir = getDocumentDir();
    let modbase = target_module.base;
    let modsize = target_module.size;
    let newmodname = target_module.name;
    let newmodpath = documentDir + "/" + newmodname + ".fid";
    let oldmodpath = target_module.path;

    console.log(`[frida-ios-dump:js]dump ${oldmodpath} to ${newmodpath}`);
    ...
    if (fmodule == -1 || foldmodule == -1) {
        console.log("[frida-ios-dump:js]cannot open file" + newmodpath);
        return null;
    }
...
function load_all_dynamic_libraries(app_path, loaded_modules) {
    console.log("[frida-ios-dump:js]: Start load all dynamic libraries... ");
    ...
    for (let i = 0, l = filenames.count(); i < l; i++) {
        ...
        if (file_name.hasSuffix_(".framework")) {
            ...
            if (bundle.isLoaded()) {
                console.log("[frida-ios-dump:js]: " + file_name + " has been loaded. ");
            } else {
                console.log("[frida-ios-dump:js]: " + file_name + " has NOT been loaded. ");
                // if (bundle.load()) {
                //     console.log("[frida-ios-dump]: Load " + file_name + " success. ");
                // } else {
                //     console.log("[frida-ios-dump]: Load " + file_name + " failed. ");
            ...
            defaultManager.fileExistsAtPath_isDirectory_(file_path, isDirPtr);
            if (Memory.readPointer(isDirPtr) == 1) {
                load_all_dynamic_libraries(file_path);
            } else {
                if (file_name.hasSuffix_(".dylib")) {
                    let is_loaded = 0;
                    for (let j = 0; j < loaded_modules.length; j++) {
                        if (loaded_modules[j].path.indexOf(file_name) != -1) {
                            is_loaded = 1;
                            console.log("[frida-ios-dump:js]: " + file_name + " has been dlopen.");
                            break;
                        }
                    } 

                    if (!is_loaded) {
                        console.log("[frida-ios-dump:js]: " + file_name + " has NOT been dlopen.");
                        // if (dlopen(allocStr(file_path.UTF8String()), 9)) {
                        //     console.log("[frida-ios-dump]: dlopen " + file_name + " success. ");
                        // } else {
                        //     console.log("[frida-ios-dump]: dlopen " + file_name + " failed. ");
...
function handleMessage(message) {
    ...
    for (var i = 0; i  < modules.length; i++) {
        ...
        console.log(`dump ${modules[i].path}|${result} done`);
...
recv('dump',function onMessage(message) {
    console.log(`[frida-ios-dump:js]cmd: ${message["payload"]}`);

    let app_path = ObjC.classes.NSBundle.mainBundle().bundlePath();
    console.log(`[frida-ios-dump:js]app path: ${app_path}`);

    let app_version = ObjC.classes.NSBundle.mainBundle().objectForInfoDictionaryKey_("CFBundleShortVersionString");
    console.log(`[frida-ios-dump:js]app version: ${app_version}`);
    let app_name = ObjC.classes.NSBundle.mainBundle().objectForInfoDictionaryKey_("CFBundleName");
    console.log(`[frida-ios-dump:js]app name: ${app_name}`);
    let app_bundle_id = ObjC.classes.NSBundle.mainBundle().objectForInfoDictionaryKey_("CFBundleIdentifier");
    console.log(`[frida-ios-dump:js]app bundle id: ${app_bundle_id}`);
    let app_display_name = ObjC.classes.NSBundle.mainBundle().objectForInfoDictionaryKey_("CFBundleDisplayName");
    console.log(`[frida-ios-dump:js]app display name: ${app_display_name}`);

    send({app_info:{
        app_version: app_version.toString(), 
        app_name: app_name.toString(), 
        app_bundle_id: app_bundle_id.toString(), 
        app_display_name: app_display_name.toString()
    }});

    let loaded_modules = get_all_app_modules();
    // load_all_dynamic_libraries(app_path, loaded_modules);

    for (let i = 0; i  < loaded_modules.length; i++) {
        let result = dump_module(loaded_modules[i]);
        if (result == null){
            console.log(`[frida-ios-dump:js]fail to dump ${loaded_modules[i].path}`);
        }else {
            console.log(`[frida-ios-dump:js]dump ${loaded_modules[i].path}|${result} done`);
            send({ dump: result, path: loaded_modules[i].path});
        }
