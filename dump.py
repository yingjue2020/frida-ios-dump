...
def generate_ipa(path, ipa_filename):
    print('[frida-ios-dump:py]generating "{}"'.format(ipa_filename))
    try:
...
def on_message(message, data):
    global g_app_info
    print(f"[frida-ios-dump:py]on_message: {message}")
    ...
    if 'payload' in message:
        ...
        if 'dump' in payload:
            ...
            scp_from = dump_path
            scp_to = PAYLOAD_PATH + '/'
            print(f"[frida-ios-dump:py]scp from: {scp_from},to: {scp_to}")
        ...
        if 'app_info' in payload:
            g_app_info = payload['app_info']
...
def open_target_app(device, name_or_bundleid):
    ...
    for application in get_applications(device):
        if name_or_bundleid == application.identifier or name_or_bundleid == application.name:
            ...
            print(f"[frida-ios-dump:py]Found {display_name} with application: {application}")
...
def start_dump(session, ipa_name):
    print('[frida-ios-dump:py]dumping {} to {}'.format(display_name, TEMP_DIR))

    script = load_js_file(session, DUMP_JS)
    script.post({"type": "dump", "payload": "hello"})
    finished.wait()

    cracked_ipa = f"{ipa_name}_cracked.ipa"
    if g_app_info is not None:
        cracked_ipa = f"{g_app_info['app_display_name']}_{g_app_info['app_version']}_cracked.ipa"
    print(f"[frida-ios-dump:py]cracked ipa: {cracked_ipa}")
    generate_ipa(PAYLOAD_PATH, cracked_ipa)
...
if __name__ == '__main__':
    ...
    if args.list_applications:
        ...
        try:
            ...
            print(f"[frida-ios-dump:py]output_ipa: {output_ipa}")
            if session:
